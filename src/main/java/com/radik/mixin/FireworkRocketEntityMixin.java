...
public abstract class FireworkRocketEntityMixin {
    ...
    private void modifyElytraBoost(LivingEntity entity, Vec3d velocity) {
        if (entity == this.shooter && entity.isGliding()) {
            ...
            double boostMulty = 0.5;
            double baseBoost = 0.03;

            Vec3d newVelocity = new Vec3d(
                    currentVel.x + rot.x * baseBoost + (rot.x * boostMulty - currentVel.x) * 0.3,
                    currentVel.y + rot.y * baseBoost + (rot.y * boostMulty - currentVel.y) * 0.3,
                    currentVel.z + rot.z * baseBoost + (rot.z * boostMulty - currentVel.z) * 0.3
            );
